  ...
  const loadModel = async () => {
    ...
    try {
      const model = await tf.loadLayersModel(
        "http://localhost:81/satellite-2/model.json"
      );
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      let top5 = Array.from(predictions)
        .map(function (p, i) {
          ...
          return {
            probability: p,
            className: TARGET_CLASSES[i], // select the value from the object
          };
        })
        .sort(function (a, b) {
        ...
        .slice(0, 1);
      console.log(top5.slice(0, 3));
      top5.forEach(function (p) {
        Output.push([index, name, p.className, p.probability]);
      });
      ...
      // let labelPrediction = results.as1D().argMax().dataSync()[0]; //get class name
      // Output.push([
      //   index,
      //   name,
      //   classNames[labelPrediction],
      //   results.dataSync()[0],
      // ]);

      // console.log("labelPrediction", labelPrediction);
    }
    setResults(Output);
  };
    ...
    .map((key) => {
      return (
        <img
          ...
          style={{ height: "375px", width: "564px", marginBottom: "55px" }}
        />
      );
    });
                          ...
                          <Box mt={7}>
                            <div className="result" key={result}>
                              <Result
                                index={result[0]}
                                filename={result[1]}
                                classLabel={"SATELLITE TYPE PREDICTION"}
                                classname={result[2]}
                                confidence={result[3]}
                              />
                            </div>
